...
const erase = () => {
  if (cell.value.num !== 0)
    log_history.value.unshift({
      status: STATES_CHANGE.CHANGE_NUMBER,
      row: row.value,
      col: col.value,
      old_value: cell.value.num
    })
  else
    log_history.value.unshift({
      status: STATES_CHANGE.CHANGE_NOTE,
      row: row.value,
      col: col.value,
      old_value: [cell.value.note]
    })
  cell.value.num = 0
  cell.value.note = []
}
  ...
  if (cell_selected.value && !cell.value.read_only) {
    if (!status_note.value) {
      if (cell.value.note.length !== 0) {
        log_history.value.unshift({
          status: STATES_CHANGE.BOTH,
          first_change: STATES_CHANGE.CHANGE_NOTE,
          row: row.value,
          col: col.value,
          old_value_note: cell.value.note
        })
        cell.value.note = []
      } else
        log_history.value.unshift({
      ...
      if (cell.value.num !== 0) {
        log_history.value.unshift({
          status: STATES_CHANGE.BOTH,
          first_change: STATES_CHANGE.CHANGE_NUMBER,
          row: row.value,
          col: col.value,
          old_value_num: cell.value.num
        })
        cell.value.num = 0
      }
      let same = false
    ...
    <div class="sudoku-wrapper">
      <div style="display:flex;flex-direction: row;justify-content: space-between">
        <div style="font-size: 20px">
          {{ $t('auto_check_mistakes') }} <input type="checkbox" v-model="auto_check_mistakes"
                                                 style="width: 20px;height: 20px">
        </div>
        <clock-panel :status="status_clock" :total-milli="totalMilli" @stop_clock="stop_clock"
                     @begin_clock="begin_clock"/>
      </div>
      <div class="game-wrapper">
        <game-panel style="width: 500px;border: 1px solid black" :challenge="challenge"
                    :answer_challenge="answer_challenge" :cell_selected="cell_selected"
                    @update_cell_selected="update_cell_selected" :auto_check_mistakes="auto_check_mistakes" :done="done"
                    :pause="pause" @continue_clock="begin_clock"/>
